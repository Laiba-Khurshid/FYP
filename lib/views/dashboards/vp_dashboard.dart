...
class VicePrincipalDashboard extends StatefulWidget {
  ...
  @override
  State<VicePrincipalDashboard> createState() => _VicePrincipalDashboardState();
}
...
class _VicePrincipalDashboardState extends State<VicePrincipalDashboard> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      drawer: CustomDrawer(
        userName: user?.fullName ?? 'Vice Principal',
        ...
        menuItems: [
          ...
          DrawerMenuItem(icon: Icons.report_problem_outlined, label: 'Escalated Complaints', onTap: () {
            Navigator.of(context).pop();
            Navigator.of(context).pushNamed(AppRoutes.complaintsScreen);
          }),
...
/// Welcome section shown at the top of the Vice Principal dashboard:
/// avatar, name, today's date, and department.
class _VicePrincipalWelcomeHeader extends StatelessWidget {
  final dynamic user;

  const _VicePrincipalWelcomeHeader({required this.user});
        ...
        children: [
          CircleAvatar(
            ...
            child: (user.profileImage == null || (user.profileImage as String).isEmpty)
                ? Text(
              (user.fullName as String).isNotEmpty ? (user.fullName as String)[0].toUpperCase() : 'V',
